...
async function promptConfiguration(): Promise<{
  ...
  console.log(
    chalk.white("      cloudflared tunnel --url http://localhost:3001"),
  );
  console.log(
    chalk.gray("   2. Your API key ready (sk_live_... or sk_test_...)\n"),
  );
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      choices: [
        {
          name: "Local (http://localhost:8080)",
          value: "http://localhost:8080",
        },
        {
          name: "Local (http://127.0.0.1:8080)",
          value: "http://127.0.0.1:8080",
        },
        {
          name: "Production (https://api.spooled.cloud)",
          value: "https://api.spooled.cloud",
        },
  ...
  // Handle custom URL
  const baseUrl =
    answers.baseUrl === "custom" ? answers.customBaseUrl : answers.baseUrl;
...
function startWebhookServer(ctx: VerificationContext): Promise<void> {
  return new Promise((resolve, reject) => {
    log.step(
      "Phase 1",
      `Starting local webhook server on port ${WEBHOOK_SERVER_PORT}...`,
    );
    ...
    server.on("error", (error: NodeJS.ErrnoException) => {
      if (error.code === "EADDRINUSE") {
        reject(
          new Error(
            `Port ${WEBHOOK_SERVER_PORT} is already in use. Please free it and try again.`,
          ),
        );
      } else {
        reject(error);
    ...
    server.listen(WEBHOOK_SERVER_PORT, () => {
      ctx.server = server;
      log.success(
        `Webhook server running on http://localhost:${WEBHOOK_SERVER_PORT}`,
      );
...
async function verifyConnectivity(ctx: VerificationContext): Promise<void> {
  ...
  try {
    ...
    if (isSpooledError(error)) {
      ...
      throw new Error(
        `API key verification failed: ${error.message} (${error.code})`,
      );
    }
    throw error;
  }
...
async function createResources(ctx: VerificationContext): Promise<void> {
  ...
  try {
    ...
    if (isSpooledError(error)) {
      throw new Error(
        `Failed to create webhook: ${error.message} (${error.code})`,
      );
    }
    throw error;
  }
...
function printSummary(
  ...
  console.log(
    chalk.gray(`  • Webhooks received: ${ctx.receivedWebhooks.length}`),
  );
...
async function main(): Promise<void> {
  ...
  console.log(chalk.bold("   🔍 SPOOLED PRODUCTION VERIFICATION TOOL"));
  console.log(
    chalk.bold(
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n",
    ),
