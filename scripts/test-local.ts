...
const SKIP_GRPC =
  process.env.SKIP_GRPC !== "0" && process.env.SKIP_GRPC !== "false";
// Skip stress tests (recommended for production)
const SKIP_STRESS =
  process.env.SKIP_STRESS === "1" || process.env.SKIP_STRESS === "true";
// Auto-detect TLS for gRPC: use TLS for production (port 443 or grpc.spooled domain)
const GRPC_USE_TLS =
  GRPC_ADDRESS.includes(":443") || GRPC_ADDRESS.includes("grpc.spooled");

if (!API_KEY) {
  console.error("❌ API_KEY environment variable is required");
  console.error(
    "   Usage: API_KEY=sk_test_... BASE_URL=http://localhost:8080 npx tsx scripts/test-local.ts",
  );
  process.exit(1);
}
...
async function testDashboard(client: SpooledClient): Promise<void> {
  ...
  await runTest("GET /api/v1/dashboard", async () => {
    ...
    log(
      `Version: ${dashboard.system?.version}, Environment: ${dashboard.system?.environment}`,
    );
  });
}
...
async function testJobIdempotency(client: SpooledClient): Promise<void> {
  ...
  await runTest(
    "Duplicate with same idempotency key returns existing",
    async () => {
      const result = await client.jobs.create({
        queueName,
  ...
  try {
    await client.jobs.cancel(firstJobId);
  } catch {
    /* ignore */
  }
}
...
async function testWorkers(client: SpooledClient): Promise<void> {
  ...
  await runTest("GET /api/v1/workers - List workers", async () => {
    ...
    assert(
      workers.some((w) => w.id === workerId),
      "should include our worker",
    );
  });
...
async function testWebhooks(client: SpooledClient): Promise<void> {
  ...
  await runTest("POST /api/v1/outgoing-webhooks - Create webhook", async () => {
    try {
      ...
      if (
        errMsg.includes("HTTP not allowed") ||
        errMsg.includes("Invalid webhook URL")
      ) {
        ssrfBlocked = true;
        log(
          "SSRF protection active - localhost webhooks blocked in production",
        );
  ...
  await runTest("GET /api/v1/outgoing-webhooks - List webhooks", async () => {
    ...
    if (!ssrfBlocked && webhookId) {
      assert(
        webhooks.some((w) => w.id === webhookId),
        "should include our webhook",
      );
    }
  });

  const skipReason = ssrfBlocked
    ? "SSRF protection blocked webhook creation"
    : undefined;
  ...
  await runTest(
    ...
    async () => {
      ...
      const webhook = await client.webhooks.get(webhookId);
      assert(
        webhook.events.includes("job.started"),
...
async function testWorkflows(client: SpooledClient): Promise<void> {
  ...
  await runTest(
    "POST /api/v1/workflows/{id}/retry - Retry workflow (should fail - not failed status)",
    async () => {
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        assert(
          message.includes("failed") || message.includes("400"),
...
async function testWorkflowExecution(client: SpooledClient): Promise<void> {
  ...
  await runTest("Only root job (A) is initially pending", async () => {
    ...
    log(
      `Job statuses: A=${jobA.status}, B=${jobB.status}, C=${jobC.status}, D=${jobD.status}`,
    );
  });
  ...
  await runTest("Job dependencies API", async () => {
    ...
    log(
      `Job D dependencies: ${deps.dependencies.map((d) => d.jobId).join(", ")}`,
    );
  });
...
async function testGrpc(client: SpooledClient): Promise<void> {
  ...
  await runTest("Connect to gRPC server", async () => {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(
        () => reject(new Error("gRPC connection timeout after 5s")),
        5000,
      );
    });
...
async function testGrpcAdvanced(client: SpooledClient): Promise<void> {
  ...
  if (SKIP_GRPC) {
    console.log(
      "  ⏭️  gRPC advanced tests skipped (set SKIP_GRPC=0 to enable)",
    );
    return;
  }
  ...
  try {
    ...
    await runTest("gRPC: Batch dequeue multiple jobs", async () => {
      ...
      assertEqual(
        deqResult.jobs.length,
        numJobs,
        `should dequeue ${numJobs} jobs`,
      );
    ...
    await runTest("gRPC: RenewLease - wrong worker ID fails", async () => {
      ...
      assertEqual(
        renewResult.success,
        false,
        "renew should fail with wrong worker",
      );
    ...
    await runTest("gRPC: Fail - no retry (to deadletter)", async () => {
      ...
      const getResult = await grpcClient.queue.getJob(enqResult.jobId);
      assert(
        getResult.job?.status === "JOB_STATUS_DEADLETTER" ||
          getResult.job?.status === "JOB_STATUS_FAILED",
    ...
    await runTest(
      "gRPC: GetQueueStats - queue with mixed statuses",
      async () => {
        if (!grpcClient) throw new Error("gRPC client not initialized");
        ...
        log(
          `Mixed queue stats: pending=${stats.pending}, processing=${stats.processing}, completed=${stats.completed}`,
        );
...
async function testGrpcErrorHandling(client: SpooledClient): Promise<void> {
  ...
  if (SKIP_GRPC) {
    console.log(
      "  ⏭️  gRPC error handling tests skipped (set SKIP_GRPC=0 to enable)",
    );
    return;
  }
  ...
  try {
    ...
    await runTest(
      "gRPC Error: Invalid queue name (special chars)",
      async () => {
        if (!grpcClient) throw new Error("gRPC client not initialized");
        ...
        try {
          ...
          assert(
            errorMsg.includes("INVALID_ARGUMENT") ||
              errorMsg.includes("alphanumeric"),
            `expected validation error, got: ${errorMsg}`,
          );
    ...
    await runTest("gRPC Error: Heartbeat for unknown worker", async () => {
      ...
      assertEqual(
        result.acknowledged,
        false,
        "should not acknowledge unknown worker",
      );
    ...
    await runTest("gRPC Error: Deregister unknown worker", async () => {
      ...
      const result = await grpcClient.workers.deregister(
      ...
      assertEqual(
        result.success,
        false,
        "should not succeed for unknown worker",
      );
  ...
  await runTest(
    ...
    async () => {
      try {
        ...
        log(
          `Queue config update failed: ${e instanceof Error ? e.message : e}`,
        );
      }
    },
  );
  ...
  await runTest(
    ...
    async () => {
      try {
        ...
        if (
          isSpooledError(e) &&
          (e.statusCode === 404 || e.statusCode === 501 || e.statusCode === 400)
        ) {
  ...
  await runTest(
    ...
    async () => {
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        log(
          `Created org: ${data.organization?.id}, name: ${data.organization?.name}`,
        );
      ...
      if (Array.isArray(deliveries) && deliveries.length > 0) {
        ...
        if (delivery && delivery.id) {
          try {
            ...
            log(
              `Retried delivery ${delivery.id}: ${result.message || "success"}`,
            );
          } catch (e: unknown) {
  ...
  await runTest("GET /api/v1/events - SSE connection test", async () => {
    ...
    try {
      const res = await fetch(
        `${BASE_URL}/api/v1/events?token=${auth.accessToken}`,
        {
...
async function testAuth(client: SpooledClient): Promise<void> {
  ...
  await runTest("POST /api/v1/auth/validate - Validate token", async () => {
    ...
    log(
      `Validate result: valid=${result.valid}, claims=${JSON.stringify(result.claims)}`,
    );
  });
...
async function testWorkerIntegration(client: SpooledClient): Promise<void> {
  ...
  await runTest("Create and start SpooledWorker", async () => {
    ...
    assert(
      workerStarted || worker.getState() === "running",
  ...
  await runTest("Process multiple jobs through worker", async () => {
    ...
    assert(
      ...
      `should process at least ${numJobs} jobs, got ${jobsProcessed}`,
    );
    assert(
      jobsCompleted >= numJobs,
      `should complete at least ${numJobs} jobs, got ${jobsCompleted}`,
    );
...
async function testOrgManagement(client: SpooledClient): Promise<void> {
  ...
  await runTest(
    "GET /api/v1/organizations/check-slug - Check slug availability",
    async () => {
      try {
        const result = await client.organizations.checkSlug(
          `test-unique-slug-${Date.now()}`,
        );
  ...
  await runTest(
    "POST /api/v1/organizations/generate-slug - Generate slug",
    async () => {
      try {
        const result = await client.organizations.generateSlug(
...
async function testOrganizationWebhookToken(
  ...
  await runTest(
    "GET /api/v1/organizations/webhook-token - Get webhook token",
    async () => {
      try {
  ...
  await runTest(
    "POST /api/v1/organizations/webhook-token/clear - Clear token",
    async () => {
      try {
...
async function testWorkflowJobs(client: SpooledClient): Promise<void> {
  ...
  await runTest(
    "GET /api/v1/workflows/{id}/jobs - List workflow jobs",
    async () => {
      try {
  ...
  await runTest(
    "GET /api/v1/workflows/{id}/jobs/{job_id} - Get specific job",
    async () => {
      try {
  ...
  await runTest(
    "GET /api/v1/workflows/{id}/jobs/status - Get jobs status",
    async () => {
      try {
        ...
        log(
          `Job statuses: ${statuses.map((s) => `${s.key}=${s.status}`).join(", ")}`,
        );
      } catch (e: unknown) {
        if (isSpooledError(e) && e.statusCode === 404) {
  ...
  await runTest(
    ...
    async () => {
      ...
      if (res.ok) {
        const data = (await res.json()) as unknown[];
        log(
          `Found ${Array.isArray(data) ? data.length : 0} organizations (admin view)`,
        );
      } else {
...
async function testEmailLogin(client: SpooledClient): Promise<void> {
  ...
  await runTest(
    "POST /api/v1/auth/email/start - Start email login",
    async () => {
  ...
  await runTest(
    "GET /api/v1/auth/check-email - Check email exists",
    async () => {
...
async function testTierLimits(mainClient: SpooledClient): Promise<void> {
  ...
  await runTest("Tier: Check current plan and usage", async () => {
    ...
    assert(
      "max_active_jobs" in limits || "maxActiveJobs" in limits,
      "should have job limit field",
    );
    assert(
      "max_queues" in limits || "maxQueues" in limits,
      "should have queues limit field",
    );
    assert(
      "max_workers" in limits || "maxWorkers" in limits,
      "should have workers limit field",
    );
    log(`Plan: ${usage.plan}, tier: ${usage.limits.tier}`);
  });
  ...
  await runTest("Tier: Verify usage tracking", async () => {
    ...
    log(
      `Usage: activeJobs=${usage.usage.activeJobs.current}/${usage.usage.activeJobs.limit}, queues=${usage.usage.queues.current}/${usage.usage.queues.limit}`,
    );
  });
  ...
  await runTest("Tier: Create fresh free tier org", async () => {
    ...
    if (res.status === 201 || res.status === 200) {
      ...
      assertEqual(
        data.organization.plan_tier,
        "free",
        "new org should be free tier",
      );
  ...
  const skipReason = orgCreationDisabled
    ? "Org creation disabled in production"
    : undefined;
  ...
  await runTest("Tier: Free org has correct limits", async () => {
    ...
    if (!loginRes.ok) {
      ...
      throw new Error(
        `Failed to login with tier org key: ${loginRes.status} - ${text}`,
      );
    }
    ...
    assertEqual(
      usage.limits.max_active_jobs,
      10,
      "free tier has 10 jobs limit",
    );
...
async function testGrpcTierLimits(client: SpooledClient): Promise<void> {
  ...
  if (SKIP_GRPC) {
    console.log(
      "  ⏭️  gRPC tier limit tests skipped (set SKIP_GRPC=0 to enable)",
    );
    return;
  }
...
async function testConcurrentOperations(client: SpooledClient): Promise<void> {
  ...
  await runTest("Concurrent worker registration", async () => {
    ...
    // Cleanup
    await Promise.all(
      results.map((r) => client.workers.deregister(r.id).catch(() => {})),
    );
  });
  ...
  await runTest("Concurrent job claim race", async () => {
    ...
    const claims = await Promise.all(
      workers.map((w) =>
        client.jobs.claim({ queueName, workerId: w.id, limit: 1 }),
      ),
    );
    ...
    if (totalClaimed === 1) {
      ...
      log(
        `Warning: ${totalClaimed} workers claimed the same job (race condition issue)`,
      );
    }
    ...
    // Cleanup
    await Promise.all(
      workers.map((w) => client.workers.deregister(w.id).catch(() => {})),
    );
  });
  ...
  await runTest("Concurrent complete attempts", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      completePromises.push(
        client.jobs
          .complete(jobId, { workerId: worker.id, result: { attempt: i } })
          .then(() => "success")
...
async function testStressLoad(client: SpooledClient): Promise<void> {
  ...
  await runTest("Mixed concurrent operations", async () => {
    ...
    assert(
      fulfilled >= rejected,
      "at least as many operations should succeed as fail",
    );
...
async function main(): Promise<void> {
  ...
  console.log(
    `   Total:     ${total} tests in ${(totalTime / 1000).toFixed(2)}s`,
  );
  console.log("═".repeat(60));
