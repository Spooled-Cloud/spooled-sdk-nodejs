...
async function main() {
  ...
  console.log(
    "   Structure: download -> [resize-small, resize-medium, resize-large]",
  );
  console.log();
    ...
    jobs: [
      {
        key: "checkout",
        queueName: "ci",
        payload: { step: "checkout", repo: "github.com/..." },
      },
      {
        key: "install",
        queueName: "ci",
        payload: { step: "install" },
        dependsOn: ["checkout"],
      },
      {
        key: "build",
      ...
      // Parallel test phase
      {
        key: "unit-tests",
        queueName: "ci",
        payload: { step: "test", type: "unit" },
        dependsOn: ["build"],
      },
      {
        key: "integration-tests",
        queueName: "ci",
        payload: { step: "test", type: "integration" },
        dependsOn: ["build"],
      },
      {
        key: "e2e-tests",
        queueName: "ci",
        payload: { step: "test", type: "e2e" },
        dependsOn: ["build"],
      },
      ...
      // Notify after deploy
      {
        ...
        payload: { step: "notify", channel: "slack" },
        dependsOn: ["deploy"],
      },
    ],
  });
  ...
  console.log(
    `   Processing ${items.length} items in parallel with final aggregation`,
  );
  console.log();
