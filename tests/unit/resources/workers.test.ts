...
describe("WorkersResource", () => {
  ...
  describe("register", () => {
    it("should register a worker", async () => {
      ...
      server.use(
        http.post(
          "https://api.spooled.cloud/api/v1/workers/register",
          async ({ request }) => {
  ...
  describe("heartbeat", () => {
    it("should send worker heartbeat", async () => {
      ...
      server.use(
        http.post(
          "https://api.spooled.cloud/api/v1/workers/worker_123/heartbeat",
          async ({ request }) => {
            receivedBody = await request.json();
  ...
  describe("deregister", () => {
    it("should deregister a worker", async () => {
      server.use(
        http.post(
          "https://api.spooled.cloud/api/v1/workers/worker_123/deregister",
          () => {
            return new HttpResponse(null, { status: 204 });
          },
        ),
