...
describe("JobsResource", () => {
  ...
  describe("boostPriority", () => {
    it("should boost job priority", async () => {
      server.use(
        http.put(
          "https://api.spooled.cloud/api/v1/jobs/job_123/priority",
          () => {
            return HttpResponse.json({
              job_id: "job_123",
              old_priority: 0,
  ...
  describe("batchStatus", () => {
    it("should get batch status", async () => {
      server.use(
        http.get(
          "https://api.spooled.cloud/api/v1/jobs/status",
          ({ request }) => {
    ...
    it("should throw for more than 100 IDs", async () => {
      ...
      await expect(client.jobs.batchStatus(ids)).rejects.toThrow(
        "Maximum 100 job IDs",
      );
    });
  });
  ...
  describe("complete", () => {
    it("should complete a job", async () => {
      server.use(
        http.post(
          "https://api.spooled.cloud/api/v1/jobs/job_123/complete",
          () => {
  ...
  describe("fail", () => {
    ...
    it("should send lease_id when provided", async () => {
      ...
      server.use(
        http.post(
          "https://api.spooled.cloud/api/v1/jobs/job_123/fail",
          async ({ request }) => {
            receivedBody = await request.json();
            return HttpResponse.json({ success: true });
          },
  ...
  describe("heartbeat", () => {
    it("should send job heartbeat", async () => {
      server.use(
        http.post(
          "https://api.spooled.cloud/api/v1/jobs/job_123/heartbeat",
          () => {
            return HttpResponse.json({ success: true });
          },
        ),
