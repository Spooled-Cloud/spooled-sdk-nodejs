...
describe("AdminResource", () => {
  ...
  it("should list organizations with X-Admin-Key header", async () => {
    ...
    server.use(
      http.get(
        "https://api.spooled.cloud/api/v1/admin/organizations",
        ({ request }) => {
  ...
  it("should update organization via PATCH", async () => {
    server.use(
      http.patch(
        "https://api.spooled.cloud/api/v1/admin/organizations/org_123",
        () => {
          return HttpResponse.json({
            id: "org_123",
            name: "Org",
    ...
    const client = createClient();
    const org = await client.admin.updateOrganization("org_123", {
      planTier: "enterprise",
    });
  ...
  it("should delete organization with hard delete flag", async () => {
    ...
    const client = createClient();
    await expect(
      client.admin.deleteOrganization("org_123", { hardDelete: true }),
