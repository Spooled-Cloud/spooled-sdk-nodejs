...
describe("SpooledClient", () => {
  describe("constructor", () => {
    ...
    it("should throw without auth", () => {
      expect(() => new SpooledClient({} as any)).toThrow(
        "requires either apiKey or accessToken",
      );
    });

    it("should throw with invalid API key format", () => {
      expect(() => new SpooledClient({ apiKey: "invalid" })).toThrow(
  ...
  describe("API calls", () => {
    ...
    it("should handle errors", async () => {
      ...
      const client = new SpooledClient({ apiKey: "sk_test_123" });
      await expect(client.jobs.get("nonexistent")).rejects.toThrow(
        "Job not found",
      );
    });
  });
...
describe("realtime JWT caching (getJwtToken)", () => {
  ...
  it("logs in only once when called twice within the token lifetime", async () => {
    ...
    const loginSpy = vi
      ...
      .mockResolvedValue(loginResponse(3600));
  ...
  it("re-logs-in when the cached token is near expiry", async () => {
    ...
    const loginSpy = vi
      ...
      .mockResolvedValueOnce(
        loginResponse(30, makeJwt(Math.floor(Date.now() / 1000) + 30)),
      )
  ...
  it("deduplicates concurrent logins into a single request", async () => {
    ...
    const loginSpy = vi
      ...
      .mockImplementation(
        () =>
          new Promise((resolve) =>
            setTimeout(() => resolve(loginResponse(3600)), 10),
          ),
      );
...
describe("JWT expiry decoding", () => {
  ...
  it("isJwtNearExpiry is false for a token far from expiry", () => {
    expect(isJwtNearExpiry(makeJwt(Math.floor(Date.now() / 1000) + 3600))).toBe(
      false,
    );
  });

  it("isJwtNearExpiry is true within the skew window and for undecodable tokens", () => {
    expect(isJwtNearExpiry(makeJwt(Math.floor(Date.now() / 1000) + 30))).toBe(
      true,
    );
