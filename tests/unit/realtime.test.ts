      ...
      await expect(
        Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("subscribe hung waiting for an ack")),
              500,
            ),
          ),
        ]),
...
describe("isAuthFailureEvent", () => {
  it("detects 401/403 from the error message text", () => {
    expect(
      isAuthFailureEvent({ message: "Unexpected server response: 401" }),
    ).toBe(true);
    expect(
      isAuthFailureEvent({
  ...
  it("returns false for non-auth errors and empty events", () => {
    ...
    expect(
      isAuthFailureEvent({ message: "Unexpected server response: 500" }),
    ).toBe(false);
...
describe("reconnect option defaults (regression)", () => {
  ...
  for (const [name, make] of [
    [
      "WebSocket",
      (o: RealtimeConnectionOptions) => new WebSocketRealtimeClient(o),
    ],
...
describe("redactToken", () => {
  it("should redact the token query param", () => {
    ...
    expect(redactToken(url)).toBe(
      "wss://api.spooled.cloud/api/v1/ws?token=***",
    );
  });

  it("should redact api_key too and preserve other params", () => {
    const url =
      "https://api.spooled.cloud/api/v1/events?api_key=sp_live_abc&foo=bar";
    expect(redactToken(url)).toBe(
      "https://api.spooled.cloud/api/v1/events?api_key=***&foo=bar",
    );
  });
});
